...
inderbar组件主要用于字母导航条，类似于手机通讯录字母条的效果，通过触摸滑动字母条可以快速检索到对应字母的内容，该组件一般和list组件一起使用，组件本身只提供字母条的样式效果。该控使用绝对定位布局，默认定位在页面右侧。由于是绝对定位，在使用的时候建议把该控件放在页面布局代码的最下面，否则会被其他控件遮罩。  
